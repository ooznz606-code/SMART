...
from analyzer_x2 import Candle, Direction, _atr, _adx_calc, _ema, _detect_sweep, _detect_displacement, _detect_fvg, _detect_order_block
...
def _run(c15, c1h, tf, mna, mxa, msc):
    ...
    for i in range(150, len(c15)-MAX_HOLD-1):
        ts=c15[i].timestamp
        ch=[c for c in c1h if c.timestamp<=ts]
        ...
        if pending:
            ...
            if bar.low<=zt and bar.high>=zb:
                ...
                if rej and (i-ltb)>=12:
                    entry=bar.close
                    stop=(zb-atr*0.22 if d==Direction.LONG else zt+atr*0.22)
                    risk=abs(entry-stop)
...
def grid_search(c15, c1h):
    grid=list(itertools.product(["15m","1H"],[15,18,22,25,28],[45,55,65],[40,50,60]))
    best={"score":-1}
    for tf,mna,mxa,msc in grid:
        if mna>=mxa: continue
...
NEW_SYMS = ["SPY", "TSLA"]
ALL_SYMS = list(CONFIGS.keys()) + NEW_SYMS

end=datetime.today(); start=end-timedelta(days=DAYS)
print("Downloading 8 symbols...")
data={}
for sym in ALL_SYMS:
    ...
    print(f"  {sym}: {len(df15)}x15m")

print("\nGrid search for SPY and TSLA...")
new_configs={}
for sym in NEW_SYMS:
    ...
    if b["score"]>0:
        ...
        print(f"  {sym}: no result, using defaults")
...
print(f"\n  {'Symbol':<7} {'wd%':>6}  {'W/T':>6}  {'WR%':>5}  ADX")
print("  "+"-"*45)
for sym,cfg in ALL_CONFIGS.items():
    tf,mna,mxa,msc=cfg
    dr=sym_dr[sym]; days=sorted(dr.keys())
    if not days: print(f"  {sym:<7}   ---  no trades"); continue
    win_d=sum(1 for d in days if "WIN" in dr[d])
    all_o=[o for v in dr.values() for o in v]
    w=all_o.count("WIN"); l=all_o.count("LOSS")
    wr=round(w/(w+l)*100,1) if (w+l)>0 else 0
    pct=round(win_d/len(days)*100,1)
    print(f"  {sym:<7} {pct:>5.1f}%  {win_d}/{len(days):>2}  {wr:>4.0f}%  {tf} {mna}-{mxa}")
